...
def test_dcpm():
    ...
    for dataset_name, dataset_info in datasets.items():
        ...
        with open(f"./outputs/dcpm/{dataset_name}.txt", 'w') as f:
            for param, (default_value, variation_range) in variations.items():
                if param == 'minRF':
                    for minRF in [round(x, 2) for x in frange(*variation_range)]:
                        ...
                        _,shuffled_memory=dcpm(default_params['minCS'], default_params['maxOR'], minRF, transactions,default_params['NM'])
                    ...
                    for minCS in [round(x, 1) for x in frange(*variation_range)]:
                        ...
                        f.flush()
                        start_time = start_benchmark()
                        _,shuffled_memory=dcpm(minCS, default_params['maxOR'], default_params['minRF'], transactions,default_params['NM'])
                        run_time, peak_memory = end_benchmark(start_time)
                        f.write(f"Run time: {run_time}s\n Peak memory: {peak_memory / 1024 / 1024}MB\n")
                        f.write(f"Shuffled memory: {shuffled_memory}\n")
                        f.flush()
                    ...
                    for maxOR in [round(x, 2) for x in frange(*variation_range)]:
                        ...
                        f.flush()
                        start_time = start_benchmark()
                        _,shuffled_memory=dcpm(default_params['minCS'], maxOR, default_params['minRF'], transactions,default_params['NM'])
                        run_time, peak_memory = end_benchmark(start_time)
                        f.write(f"Run time: {run_time}s\n Peak memory: {peak_memory / 1024 / 1024}MB\n")
                        f.write(f"Shuffled memory: {shuffled_memory}\n")
                        f.flush()
                    ...
                    for nm in range(variation_range[0],variation_range[1]+1,variation_range[2]):
                        ...
                        f.flush()
                        start_time = start_benchmark()
                        _,shuffled_memory=dcpm(default_params['minCS'], default_params['maxOR'], default_params['minRF'], transactions,nm)
                        run_time, peak_memory = end_benchmark(start_time)
                        f.write(f"Run time: {run_time}s\n Peak memory: {peak_memory / 1024 / 1024}MB\n")
                        f.write(f"Shuffled memory: {shuffled_memory}\n")
                        f.flush()
